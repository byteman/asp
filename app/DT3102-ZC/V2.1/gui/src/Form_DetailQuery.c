...
static OvertimeData  DetailVehShow;
static s8  AxleGroupShow = 0;

static void Form_DetailQuery_Timer(void *ptmr, void *parg);
static void Form_DetailQuery_Draw(LPWindow pWindow);
static void Form_DetailQuery_Proc(LPWindow pWindow, LPGuiMsgInfo pGuiMsgInfo);
...
CWindow  gWD_DetailQuery = {
    ...
    10,//5
    NULL,
    Form_DetailQuery_Timer,
    Form_DetailQuery_Draw,
    Form_DetailQuery_Proc
};

void Form_DetailQuery_Timer(void *ptmr, void *parg)
{
...
void Form_DetailQuery_Draw(LPWindow pWindow)
{
	uint16 i;
	LPControl	lpControl;
	char cBuff[36];
//    u8 ucTemp;
    ...
    if(AxleGroupShow < 0) {
		...
		sprintf(cBuff,"轴数: %d/%d",(AxleGroupShow),DetailVehShow.AxleGroupNum);
		DrawGbText(cBuff, 5, 22);
			
		sprintf(cBuff,"%d",AxleGroupShow);				//轴
		DrawGbText(cBuff, 18, 66);
			
		sprintf(cBuff,"%d",DetailVehShow.AxleWet[AxleGroupShow-1]);				//轴重
		...
		sprintf(cBuff,"%d m",(int)(DetailVehShow.AxleLen[AxleGroupShow-1] * 0.01f));				//轴距
...
void Form_DetailQuery_Proc(LPWindow pWindow, LPGuiMsgInfo pGuiMsgInfo)
{
	CControl* pControl;	
	...
	switch(pGuiMsgInfo->ID)
	{
		case WM_LOAD:
			ClearScreen();
			SysTimeDly(15);
			Get_VehShow(&DetailVehShow);
			if(DetailVehShow.AxleGroupNum) 
			{
					AxleGroupShow = 1;
			} 
			else
		...
		case WM_KEYDOWN:
			switch(pGuiMsgInfo->wParam)
			{
				...
				case KEY_RIGHT: 
					if(AxleGroupShow < DetailVehShow.AxleNum )
					{
						AxleGroupShow++;
					}		
