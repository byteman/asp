 ...
 *  静态处理
 */
static sStaticProc  StaticProcParam;
...
//sMode7AlexDyncProc SCS_ZC7_DyncProc;
...
int ZC7_Alex_Event(void* pADP)
{
	...
	{
		if(pAxle->nAlexINorRemove == 0 || pAxle->nAlexINorRemove == 1)
		{
			msg.msg_id = Msg_BackAlex;
		}
	}
...
static char ScalerRegister(void)
{
	...
	DyncZeroScaler.nPowerTime	= 2*DYNAMIC_AD_RATE;
	...
	DyncZeroBackAxle.nPowerTime	= DYNAMIC_AD_RATE/5;
	...
	DyncZeroPreAxle1.nPowerTime	= DYNAMIC_AD_RATE/5;
	...
	DyncZeroPreAxle2.nPowerTime	= DYNAMIC_AD_RATE/5;
...
#else
	ZeroTraceA.nPowerTime	= 2*STATIC_AD_RATE;
#endif
...
//			device.bitValue.axle2_valid = SCS_ZC7_DyncProc.nLeaveUpFlag;
			break;
...
void Send_SysParm_Info(char *sbuf)
{
	...
	memcpy(&sbuf[7], &gWetPar, sizeof(gWetParam));
	len += 7;
...
extern BOOL Get_NetStatus(void);
...
void Weight_Business(void *buf)
{
	...
	if(Scaler_Weight > Scaler_MaxWeight)
		Scaler_MaxWeight = Scaler_Weight;

	if(Scaler_Weight < 500)
		Scaler_MaxWeight = 0;
...
void Set_3Axle_Static_K(float k)
{
...
void Set_4Axle_Static_K(float k)
{
...
//秤台静态置零
void Scaler_Set_Zero(void)
{
	if(hs_Main == 0)
		return;
		
	if(gWetPar.ZeroTruckTick == 0)
		return;
	
	debug(Debug_Notify, "Scaler set zero!\r\n");
	ScalerStaticSetZero(hs_Main);
	ScalerDyncSetZero(hs_Main);
}
...
int32 Get_Dynamic_Weight(void)
{
	int32 tmp = 0;
	if(hs_Main != 0)
		tmp = GetScalerDyncValue(hs_Main);
...
{
	debug(Debug_Business, "set scaler weight %d\r\n", wet);
    Scaler_Weight = wet;
}
