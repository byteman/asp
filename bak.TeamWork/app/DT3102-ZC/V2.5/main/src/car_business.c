...
static void Calc_LongCar_Wet(CarInfo *pCar)
{
	...
	if((Get_FollowCarMode() != DevideAtOnce) && ((pCar->nAxleNum - pCar->nLongCarHalfAxle) > 0))
	{
		//前后两段重量相加
		wet = pCar->nLongCarHalfWet + Get_Static_Weight();
		
		//分段和在轴重和的一定范围内取分段和
		if(((int)((float)wet * 1.12f) > axlewet) && (wet < (int)((float)axlewet * 1.12f))) 
		{
			...
			if(pCar->nStatusKeepTime >= nCarWetMaxTick)
			{
				...
				if(IsSingleCarComming(pCar))
				{
					...
					if((IsLongCar() == TRUE) && (Get_LongCar_Mode() == AutoLongCarMode))
					{
    					...
    					//地感或后轴识别器判断长车失败
						pCar->nLongCarHalfAxle = pCar->nAxleNum;
						pCar->nLongCarHalfWet = 0;
...
static void Car_Business_CarWaitPayState(CarInfo *pCar, TaskMsg *pMsg)
{
	...
	switch(pMsg->msg_id) 
	{
		...
		//后地感释放
       	case Msg_GC2_Leave:
			////下秤无轴识别器if(pCar->nBackAxleNum > 1)	//有可能是前一个车离开后地感的信号
			{
				ResetTurnTick();
       			FSM_Change_CarState(pCar, stCarFarPay); 
			}
