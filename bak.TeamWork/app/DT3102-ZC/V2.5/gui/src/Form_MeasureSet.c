...
static void WetSet_Fun(LPWindow pWindow)
{
	...
	switch (CTRL_CONTENT(mWetAttr).focus) {
		...
		case 9:
			sprintf(cBuff,"请输入最大轴数");
			if( LoadEditDlg(&g_TextEditWindow, cBuff, cTemp, cTemp, 5)) 
			{
				ulTemp = atoi(cTemp);
				if ((ulTemp <= MAX_AXLE_NUM) && (ulTemp >= 2)) 
				{
					if(!Set_Max_AxleNum(ulTemp))
					{
						MsgBoxDlg(&g_MsgBoxDlg, "错误提示", "参数设置失败，请重新设置");
					}
				}
				...
				{
					snprintf(cBuff, 29, "轴数不能大于%d小于2!", MAX_AXLE_NUM);
					MsgBoxDlg(&g_MsgBoxDlg, "错误提示", cBuff);
				}
			}
...
void Form_WetSet_Proc(LPWindow pWindow, LPGuiMsgInfo pGuiMsgInfo)
{
	...
	switch(pGuiMsgInfo->ID)
	{
		...
		case WM_SHOW:
			...
			snprintf(cMaxAxleNum, 11, "%d", gWetPar.nMaxAxleNum);
