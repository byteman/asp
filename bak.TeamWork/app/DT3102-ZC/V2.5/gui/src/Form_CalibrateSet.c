...
DEF_ATTR_CTRL(mCalibrateSetAttr, &gWD_CalibrateSet, 0, 20, 240, 108, "标定系数设置",(CAttrItem *)(_attritem), sizeof(_attritem)/sizeof(CAttrItem), 0, CTRL_VISABLE);
...
void Form_CalibrateSet_Proc(LPWindow pWindow, LPGuiMsgInfo pGuiMsgInfo)
{
	...
	switch(pGuiMsgInfo->ID)
	{
		...
		case WM_SHOW:
            sprintf(DynamicCoefficientSet,"%f",gWetPar.DynamicK);
				...
				case KEY_OK:
					switch (CTRL_CONTENT(mCalibrateSetAttr).focus) 
					{
						case 0:
							...
							{
								Set_ScalerDyncStatic_K(atof(cTemp));
							}
					    break;
