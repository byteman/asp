...
static  CarInfo   *carArray = NULL;	//[MAX_CAR_NUM];
...
void CarQueue_Init(void)
{
	...
	carArray = (CarInfo *)Ccm_Malloc(MAX_CAR_NUM * sizeof(CarInfo));
	if(carArray == NULL) while(1);
...
CarInfo* CarQueue_Create_Car(void)
{
	...
	debug(Debug_Business, "车辆队列满\r\n");

	pCar = CarQueue_Get_Head();
	if(pCar != NULL)
	{
		CarQueue_Remove_Head();
		CarInfo_Reset(pCar);
		debug(Debug_Business, "创建循环车辆\r\n");
		return pCar;
	}
