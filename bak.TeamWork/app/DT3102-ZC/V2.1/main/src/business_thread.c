...
#if 1
			//使用秤台差轴重--ad1轴重
			tmpAxle->AlexDValue = (int)(tmpAxle->AlexDValue * 0.95f);
			debug(Debug_Business, "**第%d轴,轴速%d,AD1轴重:%d,AD3轴重:%d,轴峰值:%d,胎型:%s\r\n", pCar->nAxleNum, Get_CarSpeed(), tmpAxle->AlexDValue, tmpAxle->AlexKg, tmpAxle->AlexKgRef, Get_Car_TryeType(tmpAxle->bDouble));
			fPoise = 0.9f;
			if(Get_CarSpeed() > 75) fPoise = 0.8f;
			
			if(tmpAxle->AlexDValue < 2*tmpAxle->AlexKg) 
			{
				tmpAxle->AlexKg = (tmpAxle->AlexKg > (int)(tmpAxle->AlexDValue*fPoise)) ? tmpAxle->AlexKg : tmpAxle->AlexDValue;
			}
			else
			{
				//如果AD3轴重小于轴峰值的一半,车速太快，以秤台差轴重为准
				if(tmpAxle->AlexKg*2 < tmpAxle->AlexKgRef)
				{
					tmpAxle->AlexKg = (int)(tmpAxle->AlexDValue * fPoise); //车速较快,秤台波动较大
				}
			}

			debug(Debug_Business, "**保存:%d\r\n", tmpAxle->AlexKg);
#else
			debug(Debug_Business, "**第%d轴,轴速%d,轴重:%d,轴峰值:%d,胎型:%s\r\n", pCar->nAxleNum, Get_CarSpeed(), tmpAxle->AlexKg, tmpAxle->AlexKgRef, Get_Car_TryeType(tmpAxle->bDouble));
#endif
			//保存轴信息
