...
static char cFollowLevel[12] = {0};					//跟车精度处理
static char* cFollowLevelItem[] = 
{
    "上秤分车",
    "下秤分车"
};
DEF_LIST_DLG(gWD_FollowLevelSet, 20, 20, 200, 88, "分车选项", cFollowLevelItem, sizeof(cFollowLevelItem)/sizeof(char *), 1, 0);

//小车跟车精度处理
//选项一样,使用cFollowLevelItem
static char cLittleFollowLevel[12] = {0};					
DEF_LIST_DLG(gWD_LittleFollowLevelSet, 20, 20, 200, 88, "小车分车选项", cFollowLevelItem, sizeof(cFollowLevelItem)/sizeof(char *), 1, 0);
...
static const CAttrItem  _attritem[] = {
	...
	{"分车设置",cFollowLevel,20},
	{"小车分车设置",cLittleFollowLevel,21}
};

DEF_ATTR_CTRL(mSystemAttr, &gWD_SystemSet, 0, 20, 240, 108, "系统参数设置",(CAttrItem *)(_attritem), sizeof(_attritem)/sizeof(CAttrItem), 0, CTRL_VISABLE);
	...
	switch (CTRL_CONTENT(mSystemAttr).focus) {
		...
		case 5: //前轴宽度
			...
			if( LoadEditDlg(&g_TextEditWindow, "请输入前轴宽度(cm)", cTemp, cTemp, 3)) 
			{
				if(strlen(cAxleLength)) 
				{
					...
					if(ulTemp > 6000 | ulTemp < 1)
					{
							MsgBoxDlg(&g_MsgBoxDlg,"错误提示","请输入1~6000的值");
					}
					else
					{
						//ulTemp = ulTemp * 10;
		...
		case 18://调试打印等级
			if (CTRL_CONTENT(mSystemAttr).focus == 18) 
			{
				...
				Set_Debug_Level(flag, TRUE);	//设置调试打印等级
			}
			break;

		case 19://跟车选项
			if (CTRL_CONTENT(mSystemAttr).focus == 19) 
			{
				CTRL_CONTENT(gWD_FollowLevelSet).focus  = Get_FollowCarMode();
				CTRL_CONTENT(gWD_FollowLevelSet).select = Get_FollowCarMode();
				flag =	LoadListDlg(&gWD_FollowLevelSet);
				if((flag >= DevideAtOnce) && (flag <= DevideAtDown))
				{
					Set_FollowCarMode((char)flag);	
				}
			}
			break;

		case 20://小车跟车选项
			if (CTRL_CONTENT(mSystemAttr).focus == 20) 
			{
				CTRL_CONTENT(gWD_LittleFollowLevelSet).focus  = Get_LittleFollowCarMode();
				CTRL_CONTENT(gWD_LittleFollowLevelSet).select = Get_LittleFollowCarMode();
				flag =	LoadListDlg(&gWD_LittleFollowLevelSet);
				if((flag >= DevideAtOnce) && (flag <= DevideAtDown))
				{
					Set_LittleFollowCarMode((char)flag);	
				}
			}
			break;
...
void Form_WetSet_Proc(LPWindow pWindow, LPGuiMsgInfo pGuiMsgInfo)
{
	...
	switch(pGuiMsgInfo->ID)
	{
		...
		case WM_SHOW:
			...
			strncpy(cFollowLevel, cFollowLevelItem[Get_FollowCarMode()], 11);	
			strncpy(cLittleFollowLevel, cFollowLevelItem[Get_LittleFollowCarMode()], 11);	
