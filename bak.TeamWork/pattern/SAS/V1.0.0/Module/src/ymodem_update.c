...
static unsigned char *pUpdateBuffer = NULL; //[1024]
static uint32 FileCRC = 0;

void Init_YmodemUpdate_Buffer(void)
{
	pUpdateBuffer = (unsigned char *)Ccm_Malloc(1024 * sizeof(char));
}
...
static int Recv_Packet(uint32 len, uint8 sid, uint8 code)
{
	...
	//crc校验判断
	crc = Ymodem_CRC16(pUpdateBuffer, len);
	if((((u8)(crc >> 8)) != crc0) && 
		(((u8)crc) != crc1))
	{
		return 4;
	...
	while(1)
	{
		switch(state)
		{
			case 0:
				...
				for(i = 0; i < 128; i++)
				{
					if(pUpdateBuffer[i] == 0) break;
				}
				
				//获取文件长度
				rfilelen = atoi((const char *)(pUpdateBuffer + i + 1));
				if(rfilelen > 0) 
				{	
			...
			case 1:
				...
				if(len > 0)
				{
					if(!tool_flag)
					{
						...
						//匹配secureCRT
						i = len - 1;
						if((pUpdateBuffer[i] == FileDataSUB) 	&& 
							(pUpdateBuffer[i-1] == FileDataSUB)	&&
							(pUpdateBuffer[i-2] == FileDataSUB)	&& 
							(pUpdateBuffer[i-3] == FileDataSUB))
						{
							for(i = 0; i < len; i++)
							{
								if((pUpdateBuffer[i] == FileDataSUB) 	&& 
					...
					//写入flash
					if(Flash_APPBak_Store(addr, pUpdateBuffer, len) == 0) fail_flag = TRUE;
					addr += len;	

					//计算升级文件crc校验值					
					FileCRC = CRC32H_Chack8(0, pUpdateBuffer, len);
				}	
				Usart1_WriteData(&MODEM_ACK, 1);
				break;
			...
			case 2:
				...
				if(!fail_flag)
				{
					...
					Flash_UPFlag();
					sprintf((char *)pUpdateBuffer, ">>升级文件CRC: %x,长度: %d\r\n", FileCRC, filelen);
					Usart1_WriteData((const char *)pUpdateBuffer, strlen((const char *)pUpdateBuffer));
					return 0;
				}
