...
#define  CRC_FILE		"DT3102.txt"			//crc file
#define  BIN_FILE		"DT3102.bin"
...
extern void TIM5_TimeISR_Open(void);
extern void TIM5_TimeISR_Close(void);

extern void TIM6_TimeISR_Open(void);
extern void TIM6_TimeISR_Close(void);

extern void TIM7_TimeISR_Open(void);
extern void TIM7_TimeISR_Close(void);

//为提高USB升级成功率，关闭部分频繁中断和高优先级任务
static void busy_business_sleep(void)
{
	//关闭AD采集
	TIM6_TimeISR_Close();
	//2ms定时器
...
static void busy_business_wakeup(void)
{
	//开启AD采集
	TIM6_TimeISR_Open();
	//2ms定时器
...
static u8 USB_updata(void)
{
	...
	//网络升级时不能优盘升级
	if(Get_NetUpdate_Flag()) return 0;
    
	//关闭软件看门狗监测, 关闭波形与调试信息发送
...
out:
	//唤醒睡眠的资源
	busy_business_wakeup();
	f_close(&fp);		//close file
	f_mount(0, NULL); 	//umount fat32
    
    SysTimeDly(10);     //延时等待算法线程运行
