...
static SYS_STK *pBeepStackBottom = NULL;
...
static s8 BEEPJOB_Open(u32 lParam)
{
	...
	pBeepStackBottom = (SYS_STK *)Ccm_Malloc(BeepJobStackSize * sizeof(SYS_STK));
	pBeepStackBottom += (BeepJobStackSize-1);

	TaskStatus |= SysTaskCreate(BeepJob_Default,						//任务函数
										(void *)0,														//任务参数
										pBeepStackBottom, /*&BeepJobStack[BeepJobStackSize - 1]*/	//任务堆栈
										BeepJob_PRIO);												//任务优先级
