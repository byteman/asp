...
static volatile FollowCarMod FollowMode = DevideAtOnce;
static volatile FollowCarMod LittleFollowMode = DevideAtOnce;
...
{	
	if(((signed char)type < 0) || (type > DevideAtDown)) return;
...
{
	return LittleFollowMode;
}

void Set_LittleFollowCarMode(char type)
{	
	if(((signed char)type < 0) || (type > DevideAtDown)) return;

	LittleFollowMode = (FollowCarMod)type;
	Param_Write(GET_U8_OFFSET(ParamLittleFollowCarMode), &type, sizeof(uint8));
}
...
//	BOOL err = FALSE;
	...
	if((unsigned char)FollowMode > DevideAtDown) FollowMode = DevideAtOnce;
