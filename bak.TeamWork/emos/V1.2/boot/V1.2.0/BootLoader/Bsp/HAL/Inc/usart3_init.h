...
//串口设置u8 USART3_Config(USART_STRUCT* Real);					//按配置初始化串口
void USART3_Init(const u32 BaudRate_t);				//设置波特率
	
//功能函数
void USART3_RX_Buffer_Clear(void);						//清空缓冲区

//状态查询	
u32 LookUSART3_GetBuffCount(void);						//获取缓冲区中接收到的数据个数
u8 USART3CheckISRSendBusy(void);							//中断发送忙状态检测

//接收数据
u8 USART3_GetByte(u8* Data);									//缓冲区中提取单字符数据
u32 USART3_GetString(u8* Data ,const u32 len);			//缓冲区中提取字符串数据
u8 USART3_GetByte_WaitTime(u8* Data,const u32 TimeLater);		//时间内提取字符数据

//发送数据
void USART3_SendByte(const u8 Data);								//单字符发送
void USART3_SendString(u8* Data,const u32 Len);			//多字符输出
void USART3_DMASendString(const u32 Len);						//DMA方式多字符输出
