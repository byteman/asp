...
//接收控制队列
static	u8*			USART6QStart = (u8*)0;		      //开始位置
static	u8*			USART6QEnd = (u8*)0;		        //结束位置
static  u8*			USART6QIn = (u8*)0;				    	//输入位置
static	u8*			USART6QOut = (u8*)0;				    //输出位置
static  u32			USART6QDataCount = 0;           //空间里数据个数 

//串口配置
static  USART_STRUCT*		USART6Real = (USART_STRUCT*)0;				//串口结构体指针
static 	u8 			USART6_GetChar = 0;					//接收到的单个字符				中断中使用	
static  u32			ISRSendLen = 0;							//中断需要发送的数据个数
static  u32			ISRSendCnt = 0;							//中断发送个数
static  u8			ISRSendBusyFlag = 0;				//中断发送忙状态	1忙	0不忙

/****************************************************************************
* 名	称：void USART6_GPIO_Init(void)
* 功	能：串口引脚初始化
* 入口参数：无
* 出口参数：无
* 说	明：无
...
/****************************************************************************
* 名	称：void USART6_Init(const u32 BaudRate_t)
* 功	能：串口初始化
...
/****************************************************************************
* 名	称：void USART6_NVIC_Init(void)
* 功	能：串口中断向量表初始化
* 入口参数：无
* 出口参数：无
* 说	明：无
...
/****************************************************************************
* 名	称：void USART6_RX_Buffer_init(void)
* 功	能：串口接收初始化
* 入口参数：无
* 出口参数：无
* 说	明：接收中断与接收缓冲区绑定
...
{
	USART6QStart = USART6Real->USARTRxBuffer;						//开始位置
	USART6QEnd = &USART6Real->USARTRxBuffer[USART6Real->USARTRxBufferSize - 1];	//结束位置
	USART6QIn = USART6QStart;														//输入位置=开始位置
	USART6QOut = USART6QStart;													//输出位置=开始位置
	USART6QDataCount = 0;																//空间里数据个数
}

/****************************************************************************
* 名	称：u8 USART6_Config(USART_STRUCT* Real)
* 功	能：串口设置
...
u8 USART6_Config(USART_STRUCT* Real)
{
	...
	USART6Real = Real;						//结构体获取
	
	USART6_Init(USART6Real->BaudRate);	//串口初始化
	USART6_GPIO_Init();						//串口引脚初始化
	USART6_NVIC_Init();						//中断初始化
	USART6_RX_Buffer_init();			//接收中断与接收缓冲区绑定
...
/****************************************************************************
* 名	称：void USART6_RX_Buffer_Clear(void)	
* 功	能：清空接收缓冲区
* 入口参数：无
* 出口参数：无
* 说	明：无						
...
/****************************************************************************
* 名	称：u32 LookUSART6_GetBuffCount(void)
* 功	能：获取缓冲区中接收到的数据个数
* 入口参数：无
* 出口参数：无
* 说	明：无					  
...
{
	if(USART6QDataCount != 0)				//有未处理数据存在
	{
		*Data = *USART6QOut;					//从队列中取数据
		USART6QOut++;									//指向下一个未处理数据点
		if(USART6QOut > USART6QEnd)		//到了最后一个判断点则跳回开始点
			USART6QOut = USART6QStart;	
		USART6QDataCount--;						//未处理数据个数递减
...
/****************************************************************************
* 名	称：u8 USART6_GetByte_WaitTime(u8* Data,const u32 TimeLater)	
* 功	能：u8* Data 接收到的数据
...
u8 USART6_GetByte_WaitTime(u8* Data,const u32 TimeLater)
{
	...
	while(1)
	{
		if(LookUSART6_GetBuffCount() == 0)	//缓冲区中没有数据
		{	
			err++;
...
/****************************************************************************
* 名	称：u32 USART6_GetString(u8* Data ,const u32 len)	
* 功	能：缓冲区中提取字符串数据
* 入口参数：u8* Data 接收到的数据
...
u32 USART6_GetString(u8* Data ,const u32 len)
{
	...
	if(USART6QDataCount == 0)					   	//如果缓冲区没有数据
		return 0;									   				//接收失败

	if(cnt > USART6QDataCount)				   	//要取的个数比目前接收的个数还多
		cnt = USART6QDataCount;				   		//取所接收到的所有数据

	if((USART6QOut + cnt - 1) > (USART6QEnd + 1))		//有部分数据在缓冲区的前端		-1从0开始存储的
	{
		i = USART6QEnd - USART6QOut + 1;							//后半部分的数据个数
		memcpy(Data , USART6QOut , i);								//取后N个数据
		memcpy(Data + i , USART6QStart , cnt - i);	 	//取前N个数据
		USART6QOut = USART6QStart + cnt - i;	 				//更新取数位置
	}
	else
	{
		memcpy(Data,USART6QOut,cnt);	 		//取N个数据
		USART6QOut += cnt;								//指向下一个未处理数据点
	}

	if(USART6QOut > USART6QEnd)	 				//输出位置超出最后的一个地址
		USART6QOut = USART6QOut - USART6QEnd + USART6QStart - 1;	   //-1从0开始	//计算输出地址
	USART6QDataCount -= cnt;			 			//减掉接收了的数据个数
	
	return cnt;								 //返回获取的个数
}

/****************************************************************************
* 名	称：void USART6_SendByte(const u8 Data)
* 功	能：单字符发送
* 入口参数：const u8 Data 	发送单字符数据
* 出口参数：无
* 说	明：无				   
****************************************************************************/
void USART6_SendByte(const u8 Data)		   //单字符数据输出
{
...
/****************************************************************************
* 名	称：void USART6_SendString(u8* Data,const u32 Len)
* 功	能：多字符输出
* 入口参数：u8* Data 	发送的数据
						const u32 Len				字符个数
* 出口参数：无
...
void USART6_SendString(u8* Data,const u32 Len)		//多字符输出
{
	...
	if(Len > USART6Real->USARTTxBufferSize)			//缓冲区长度是否足够
		return;
	
	memcpy(USART6Real->USARTTxBuffer,Data,Len);	//拷贝数据到缓冲区
	
	ISRSendLen = 0;									//清除需要发送的数据个数
	ISRSendLen = Len;								//记录需要发送的数据个数
	ISRSendBusyFlag = 1;						//开启忙状态
	
	USART_SendData(USART6, USART6Real->USARTTxBuffer[0]);	//发送第一个字节			
	ISRSendCnt = 1;									//发送计数
	USART_ITConfig(USART6, USART_IT_TXE, ENABLE);		//开启发送中断	
}

/****************************************************************************
* 名	称：u8 USART6CheckISRSendBusy(void)	
* 功	能：中断发送忙状态检测
* 入口参数：无
* 出口参数：u8		1忙	0不忙
* 说	明：上一次的发送是否完成，忙则等待
...
/****************************************************************************
* 名	称：void USART6_IRQHandler(void)	
* 功	能：中断机制
* 入口参数：无
* 出口参数：无
* 说	明：接收到的数据存入接收缓冲区
...
{	
	...
	{
		//数据接收动作
		USART_ClearITPendingBit(USART6, USART_IT_RXNE);				//清空接收中断标志

		USART6_GetChar = USART_ReceiveData(USART6);								//接收到的字符数据
		
		if(USART6QDataCount < USART6Real->USARTRxBufferSize)			//如果空间未满,保存数据 
		{
			if(USART6QIn > USART6QEnd)			//如果有空间可存数据，但输入位置在末尾位置
				USART6QIn = USART6QStart;			//将输入位置跳到开始位置
			*USART6QIn = USART6_GetChar;		//接收到的字符数据存入缓冲区输入位置
			USART6QIn++;										//输入位置增加
			USART6QDataCount++;							//空间里数据个数增加	
		}
	}
	
	else if(USART_GetFlagStatus(USART6, USART_IT_ORE) == SET)		//检测是否有接收溢出
		USART_ReceiveData(USART6);									//清接收溢出标志，只能用读数据的方式来清溢出标志
	...
	{		
		...
		{
			USART_ITConfig(USART6, USART_IT_TXE, DISABLE);		//禁止发送移位缓冲区空中断
