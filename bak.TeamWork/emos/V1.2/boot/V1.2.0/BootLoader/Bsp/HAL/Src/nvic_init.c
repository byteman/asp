#include "nvic_init.h"
#include "stm32f4xx.h"		//固件库函数

#define AppRunAddr				0x08000000		//中断向量表地址
...
void nvic_init(void)
{
		...
		NVIC_SetVectorTable(NVIC_VectTab_FLASH, AppRunAddr);   	//使用
	#endif

	//嵌套向量中断控制器设置
	NVIC_PriorityGroupConfig(NVIC_PriorityGroup_1);			//优先级分组号 0-4
