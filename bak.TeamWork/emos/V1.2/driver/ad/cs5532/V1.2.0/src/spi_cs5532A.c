...
void CS5532A_Init(void)
{
  ...
  SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_16;//16分频后5.25Mhz
...
{
	CS5532A_SendByte(CS5532Ax,ConfigRegister+WriteRegister);					//配置寄存器
	CS5532A_SendWord(CS5532Ax,SETUP1_VRS);									//参考电压		 	2.5V < Vref <= VA+
}
...
{
	CS5532A_SendByte(CS5532Ax,ChannelRegister + ChannelSet_1 + WriteRegister);	//通道寄存器的通道配置寄存器1
	CS5532A_SendWord(CS5532Ax,SETUP1+SETUP2);									//写通道配置
}
...
static BOOL CS5532A_Start(u8 CS5532Ax,u8 flag,u8 SetupNum)
{
	...
	if (flag == 0)
		CS5532A_SendByte(CS5532Ax,StartConvert+SetupX+Single);			//发送单次转换命令
	else
		CS5532A_SendByte(CS5532Ax,StartConvert+SetupX+Continuation);	//发送连续转换命令
...
u8 CS5532A1_Flag(void)
{
	...
	CS5532A1_CS_LOW();	
	CS5532_CS_Delay(NS_50);
	if( Bit_RESET == CS5532A_READ_MISO() )  //转换完成
		return 1;
	else
		return 0;
}
...
u8 fStepConfgAD_Ax( u8 u8Channel , u8 Step )
{
	...
	if( 1 == Step )		//复位
	{
		CS5532A_SetCMD(u8Channel);			   					 //设置成命令模式

		CS5532A_SendByte(u8Channel,ConfigRegister+WriteRegister);//对配置寄存器进行写操作
		CS5532A_SendWord(u8Channel,RS1);						 //复位,进入命令模式
		return true ;
	}
	//延时20毫秒后
	if( 2 == Step )		//配置
	{
		...
		if(u8Channel & ret & CS5532A1) {
			temp = CS5532A_ReadWord(CS5532A1);					    //RV为1 0x10000000 复位操作结束	读取一次后RV自动为0
			if(temp!=0x10000000)
				ret &= ~CS5532A1;								    //复位标志不正确
		}
		if(u8Channel & ret & CS5532A2) {
			temp = CS5532A_ReadWord(CS5532A2);				        //RV为1 0x10000000 复位操作结束	读取一次后RV自动为0
			if(temp!=0x10000000)
				ret &= ~CS5532A2;								    //复位标志不正确
		}
		if(!ret) {
			return ret;
		}
	...
	CS5532A_ConfigRegister(ret);	//设置	  配置寄存器	  参考电压
	CS5532A_ChannelRegister(ret);	//设置	  通道寄存器	  物理通道 + 增益 + 字速率 + 单双极 + 锁存位 + 延时 +  开路检测 +  偏移/增益物理通道选择

	CS5532A_Start(ret,1,2);			//开启连续转换	
