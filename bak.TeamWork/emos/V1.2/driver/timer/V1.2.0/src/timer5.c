...
static u32 TIM5_Count = 0;			//计数器
static u8 TIM5_CntFlag = 0;			//计数器开启标志

static TIM5_fun RealTim5fun = 0;		//定时器5执行函数指针			必须static
static u8 TIM5_pDisc[] = "LuoHuaiXiang_Tim5\r\n";		//设备描述字符

/****************************************************************************
* 名	称：void TIM5_TimeOpen(void)
* 功	能：开启定时器
* 入口参数：无
* 出口参数：无
* 说	明：无
...
{
	TIM_Cmd(TIM5, ENABLE);		//调用固件库函数
}

/****************************************************************************
* 名	称：void TIM5_TimeClose(void)
* 功	能：关闭定时器
* 入口参数：无
* 出口参数：无
* 说	明：无
...
{
	TIM_Cmd(TIM5, DISABLE);		//调用固件库函数
}

/****************************************************************************
* 名	称：void TIM5_ITOpen(void)
* 功	能：开启定时器中断
* 入口参数：无
* 出口参数：无
* 说	明：中断开启计时值增加
...
{
	TIM_ITConfig(TIM5, TIM_FLAG_Update, ENABLE);	   //打开TIM5的更新标记TIM_FLAG_Update中断
}
...
{
	TIM_ITConfig(TIM5, TIM_FLAG_Update, DISABLE);	   //关闭TIM5的更新标记TIM_FLAG_Update中断
}

/****************************************************************************
* 名	称：void TIM5_TimeISR_Open(void)
* 功	能：计时中断开启
* 入口参数：无
* 出口参数：无
* 说	明：无
...
void TIM5_TimeISR_Open(void)
{
	...
	if(TIM5_CntFlag == 1)								//如果需要计数器计数
	{
		if(TIM5_Count == 0)								//第一次开始时
			TIM5_Count = 1;									//计时到达获取值时，最后一个中断计数还没计数
	}
}

/****************************************************************************
* 名	称：void TIM5_TimeISR_Close(void)
* 功	能：计时中断关闭
* 入口参数：无
* 出口参数：无
* 说	明：无
...
/****************************************************************************
* 名	称：void TIM5_FlagOnOff(u8 OnOff)
* 功	能：开启关闭计数器
...
{
	if(OnOff > 0)
		TIM5_CntFlag = 1;		 //计时值复位
	else
		TIM5_CntFlag = 0;		 //计时值复位
}

/****************************************************************************
* 名	称：void TIM5_Caller(void(*TIM7_fun)(void))
* 功	能：TIM7回调函数
* 入口参数：回调执行函数
* 出口参数：无
* 说	明：无
...
{
	RealTim5fun = TIM5_fun_t;	//执行函数指针指向执行函数
}

/****************************************************************************
* 名	称：void TIM5_DAC_IRQHandler(void)
* 功	能：定时器中断处理
* 入口参数：无
* 出口参数：无
* 说	明：计时值增加
...
void TIM5_IRQHandler(void)
{
	...
	TIM_ClearITPendingBit(TIM5, TIM_FLAG_Update);	//清空溢出中断标志
	if(TIM5_CntFlag == 1)
	{
		if(TIM5_Count == 0xFFFFFFFF)
			TIM5_Count = 0;							//超过最大值重新回到0开始计数
		TIM5_Count++;									//计时值增加
	}
	
	if(RealTim5fun != 0)								//如果执行函数有指向
		RealTim5fun();										//执行执行函数
...
/****************************************************************************
* 名	称：static s8 TIM5_Open(u32 lParam)
* 功	能：统一接口
...
/****************************************************************************
* 名	称：static s32 TIM5_Read(u8* buffer,u32 len,u32* Reallen,u32 lParam) 
* 功	能：串口读
...
/****************************************************************************
* 名	称：static s32 TIM5_Write(u8* buffer,u32 len,u32* Reallen,u32 lParam) 
* 功	能：串口读
* 入口参数：u8* buffer		接收缓冲区
						u32 len				希望发送的长度
						u32* Reallen	实际发送的长度
...
static s32 TIM5_Ioctl(u32 cmd,u32 lParam)
{
	switch(cmd)
	{
		case TIM5_CMDClearCnt:	//清空计数器
		{
			TIM5_ResetCount();
			return 1;							//成功
		}
		case TIM5_CMDTimOpen:		//打开定时器并响应中断
		{
			TIM5_TimeISR_Open();
			return 1;							//成功
		}
		case TIM5_CMDTimClose:	//关闭定时器并关闭中断
		{
			TIM5_TimeISR_Close();
			return 1;							//成功
		}
		case TIM5_CMDSetCntFlag:	//关闭定时器并关闭中断
		{
			TIM5_FlagOnOff((u8)lParam);
			return 1;							//成功
		}
		case TIM5_CMDSetCaller:	//设置回调函数
		{
...
* 说	明：无
****************************************************************************/
u8 TIM5_Config(TIM5_fun Cfun,u16 TimeUS,u8 mode)				//默认为毫秒中断
{
	//注册用变量
	DEV_REG tim5 = 			//设备注册信息表						不必static		
	{
		CHAR_TIM5,					//设备ID号
		...
		TIM5_pDisc,					//驱动描述			自定义
		20120001,						//驱动版本			自定义
		(u32*)TIM5_Open,		//设备打开函数指针
		(u32*)TIM5_Close, 	//设备关闭函数指针
		(u32*)TIM5_Read,		//字符读函数
		(u32*)TIM5_Write,		//字符写函数
		(u32*)TIM5_Ioctl		//控制函数
	};
	...
	//定时器配置
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM5, ENABLE);	//给定时器分配时钟APB1	//APB1预分频为非1时,定时器时钟频率为2倍频即36*2=72MHZ
  TIM_DeInit(TIM5);																			//复位TIM2定时器到默认值
  ...
  TIM_TimeBaseInit(TIM5, &T);									//根据上面设置初始化定时器TIM2
  TIM_ClearFlag(TIM5, TIM_FLAG_Update);				//清空溢出中断

	//嵌套向量中断控制器设置	  
	N.NVIC_IRQChannel = TIM5_IRQn;							//配置对TIMx_IRQn的中断监听
  ...
  NVIC_Init(&N); 															//根据上面设置初始化中断控制器
	...
	if(Cfun != 0)																//回调函数不为空
		TIM5_Caller(Cfun);												//设置回调函数
	
	if(DeviceInstall(&tim5) != HVL_NO_ERR)			//设备注册
		return 0;
