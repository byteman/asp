...
#define TIM6_CMDClearCnt			0				//清空计数器
#define TIM6_CMDTimOpen				1				//开启定时器启用中断响应
#define TIM6_CMDTimClose			2				//关闭定时器关闭中断响应
#define TIM6_CMDSetCntFlag		3				//设置计数开关
#define TIM6_CMDSetCaller			4				//设置回调函数

typedef void(*TIM6_fun)(void); 			//函数指针
	
/****************************************************************************
* 名	称：u8 TIM7_Config(TIM6_fun Cfun,u16 TimeUS,u8 mode)
* 功	能：定时器配置
...
* 说	明：无
****************************************************************************/
u8 TIM6_Config(TIM6_fun Cfun,u16 TimeUS,u8 mode);	 	//定时器初始化
...
* 说	明：
命令								参数							返回值
TIM6_CMDClearCnt					无用							无用
TIM6_CMDTimOpen						无用							无用
TIM6_CMDTimClose					无用							无用
TIM6_CMDSetCntFlag				1开0关						无用
TIM6_CMDSetCaller					函数地址					无用
****************************************************************************/	
...
void TIM6_CallerTest2(void)
{
	...
	HANDLE TIM6_Dev = 0;													//设备句柄
	
	TIM6_Config(TIM6_CallerTest1,10,Mode_500US);	//500微秒中断一次	10次激活一个中断响应
	
	TIM6_Dev = DeviceOpen(CHAR_TIM6,0,&err);			//设备打开
	if(err != HVL_NO_ERR)
		while(1);
	
	if(DeviceIoctl(TIM6_Dev,TIM6_CMDSetCntFlag,1) == 0)		//设置计数开关
		while(1);
	...
	if(DeviceIoctl(TIM6_Dev,TIM6_CMDSetCaller,(u32)&TIM6_CallerTest2) == 0)			//切换回调函数
		while(1);
	
	SysTimeDly(5000);																			//5S
	
	if(DeviceIoctl(TIM6_Dev,TIM6_CMDTimClose,0) == 0)			//开启定时器启用中断响应
		while(1);
	
	if(CharDeviceRead(TIM6_Dev,0,0,&cnt,0) == 0)					//获取计数值
		while(1);
	
	if(DeviceIoctl(TIM6_Dev,TIM6_CMDClearCnt,0) == 0)			//复位计数器
		while(1);
*/
