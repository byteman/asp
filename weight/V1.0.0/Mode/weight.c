...
{
	if((err >= ERR_MAX) || (err < 0)) return NULL;
...
static ERR_WEIGHT AdcChanleProc(HScaler hs)
{
    ...
    for(chanle = 0 ; chanle < SCALER(hs)->nAdcCount; chanle++)
    {
        ...
        pAdcChanle->fFilterValue = pAdcChanle->pFilter(pAdcChanle->pFcb_UserData, (float)(pAdcChanle->nOrgAd));
...
ERR_WEIGHT ScalerStaticSetZero(HScaler hs)
{
	...
	SCALER(hs)->StaticScalerImpl.pSetZero(SCALER(hs)->StaticScalerImpl.pZtcb_UserData);
...
ERR_WEIGHT ScalerDyncSetZero(HScaler hs)
{
	...
	for(i = 0; i < SCALER(hs)->nAdcCount; i++)
	{
		...
		pAdcChanle->pSetZero(pAdcChanle->pZcb_UserData);
