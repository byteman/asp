...
#define	MATRIX_KEY_LINE	6//键盘行数
#define	MATRIX_KEY_ROW	5//键盘列数
...
static u16 KeyPin = 0;	
static GPIO_TypeDef* KeyPort = (GPIO_TypeDef*)0;

static u32 Matrix_Key_IntEnable(BOOL bEn);
static const char KeyTable[MATRIX_KEY_LINE][MATRIX_KEY_ROW] = {
//		A				B			C			D				E			
/*1*/	KEY_7,			KEY_8,		KEY_9,		KEY_Del,		KEY_4,		
/*2*/	KEY_5,			KEY_6,		KEY_UNIT,  	KEY_1,			KEY_2,		
/*3*/	KEY_3,			KEY_QUERY,	KEY_ADD,	KEY_0,			KEY_DOT,			
/*4*/	KEY_PRINT,		KEY_UP,		KEY_LEFT,	KEY_MENU,	    KEY_RIGHT,		
/*5*/	KEY_BACK,		KEY_DOWN,   KEY_OK,    	KEY_CAR,		KEY_AXIS,			
/*6*/	KEY_DIAGNOSIS,	KEY_ZERO,	KEY_ERR,	KEY_ERR,		KEY_ERR
};
//矩阵键盘中断注册信息表
...
static u32 Matrix_Key_IntEnable(BOOL bEn)
{
	...
	//外部中断配置
    EXTI_InitStructure.EXTI_Line = KEY_L1_PIN | KEY_L2_PIN | KEY_L3_PIN | KEY_L4_PIN | KEY_L5_PIN ;
...
static u8 KeyScanCheckL(void)
{
	if(0 == KEY_GPIO_STATE_GET(KeyPort, KeyPin)){
		switch(KeyPin){
			case KEY_L1_PIN:return 0;
			case KEY_L2_PIN:return 1;
			case KEY_L3_PIN:return 2;
			case KEY_L4_PIN:return 3;
			case KEY_L5_PIN:return 4;
...
* 说	明：得到矩阵键盘的行列值
			未使用中断
****************************************************************************/	
static BOOL KeyCheck(u8* K_H,u8* K_L)
{
	if( (*K_L = KeyScanCheckL()) >= MATRIX_KEY_ROW){		//找列号 如果列值不正确
		return FALSE;
	}	
	SysTimeDly(10);											//100毫秒延时
