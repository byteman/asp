		...
		case WM_KEYDOWN:
			switch(pGuiMsgInfo->wParam)
			{
				...
				case VK_9:												//插入数据
					...
					pEdit->arrDigit[pEdit->nCurPos] =  0x30 + pGuiMsgInfo->wParam - VK_0;
					pEdit->nCurPos++;
					pEdit->nCurLen++;
					guiMsgInfo.ID = WM_SHOW;
					GuiMsgQueuePost(&guiMsgInfo);
...
// 				case VK_DECIMAL:			//符号，小数点(将来有输入法时作为调符号功能)
// 					if(pEdit->nCurLen >= pEdit->nMaxAllowed) return 0;	//最大位数限制
...
// 				case VK_SIGN:			//符号，负号(将来有输入法时作为输入法切换功能)
// 					break;

				case VK_DELETE: 			//删除光标处的数据
...
DEF_TEXT_EDIT(m_TextEdit, &g_TextEditWindow, 8, 32, 184, 28, CTRL_VISABLE, NULL, 16, NULL, TA_CENTER);
	...
	while(TRUE)
	{
		...
		if(err == SYS_NO_ERR) 
		{
			...
			else
			{
				...
				if((guiMsgInfo.ID == WM_KEYDOWN) && (guiMsgInfo.wParam == VK_OK))
				{
